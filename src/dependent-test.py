...
from typing import List, Optional, Dict, Tuple, Any  # 新增 Any 导入
import json
...
class GraphQLPathFinder:
    ...
    # ---------------------- 新增功能：获取所有对象类型名称 ----------------------
    def _get_all_object_type_names(self) -> List[str]:
        """
        提取 Schema 中所有 GraphQLObjectType 类型的对象名称（去重）
        排除内置标量类型，仅保留自定义对象类型
        """
        object_type_names = []
        for type_name, graphql_type in self.schema.type_map.items():
            # 仅筛选对象类型，排除标量/接口/联合等类型
            if isinstance(graphql_type, GraphQLObjectType):
                object_type_names.append(type_name)
        # 去重并返回
        return list(set(object_type_names))

    # ---------------------- 新增功能：遍历所有对象的最短路径及长度 ----------------------
    def find_all_objects_shortest_paths(self, max_depth: int = 4) -> Dict[str, Dict[str, Any]]:
        """
        遍历 Schema 中所有对象类型，计算每个对象的最短路径及长度
        :param max_depth: 最大搜索深度
        :return: 结构化结果，格式：
            {
                "ObjectName": {
                    "shortest_length": 最短路径长度（根类型为0）,
                    "paths": 最短路径的紧凑格式列表,
                    "all_paths": 该对象的所有路径紧凑格式列表（可选）
                }
            }
        """
        # 1. 获取所有对象类型名称
        all_object_names = self._get_all_object_type_names()
        # 2. 获取根类型（Query/Mutation），根类型自身路径长度为0
        root_types = []
        if self.schema.query_type:
            root_types.append(self.schema.query_type.name)
        if self.schema.mutation_type:
            root_types.append(self.schema.mutation_type.name)
        
        result = {}
        for obj_name in all_object_names:
            # 处理根类型：自身就是起点，路径长度为0
            if obj_name in root_types:
                result[obj_name] = {
                    "shortest_length": 0,
                    "paths": [obj_name],
                    "all_paths": [obj_name]
            ...
            # 3. 查找当前对象的所有路径
            obj_paths = self.find_paths_to_object(obj_name, max_depth=max_depth)
            if not obj_paths:
                # 无可用路径
                result[obj_name] = {
            ...
            # 4. 计算最短路径长度（所有路径中最小的长度）
            path_lengths = [len(path) for path in obj_paths]
            shortest_length = min(path_lengths)
            
            # 5. 筛选出所有最短长度的路径
            shortest_paths = [path for path in obj_paths if len(path) == shortest_length]
            
            # 6. 转换为紧凑格式（便于阅读）
            compact_shortest_paths = self.format_compact_paths(shortest_paths)
            compact_all_paths = self.format_compact_paths(obj_paths)
            
            # 7. 保存结果
            result[obj_name] = {
                "shortest_length": shortest_length,
                "paths": compact_shortest_paths,
                "all_paths": compact_all_paths
            }
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--object", help="The target object name (optional, if not set, traverse all objects).")
    ...
    if args.object:
        ...
        print(f"\nFound {len(formatted_paths)} path{'s' if len(formatted_paths) != 1 else ''}:")
        for path in formatted_paths:
            print(f"- {path}")
    else:
        # 新增逻辑：遍历所有对象并输出最短路径
        all_objects_paths = path_finder.find_all_objects_shortest_paths(args.max_depth)
        
        print("=== All Objects Shortest Paths ===\n")
        # 按最短路径长度排序输出
        sorted_objects = sorted(
            all_objects_paths.items(),
            key=lambda x: x[1]["shortest_length"] if x[1]["shortest_length"] is not None else 999
        )
        
        for obj_name, path_info in sorted_objects:
            print(f"🔹 Object: {obj_name}")
            if path_info["shortest_length"] is None:
                print("  - No valid paths found\n")
                continue
            print(f"  - Shortest path length: {path_info['shortest_length']}")
            print(f"  - Shortest paths:")
            for p in path_info["paths"]:
                print(f"    - {p}")
            # 可选：输出该对象的所有路径
            # print(f"  - All paths:")
            # for p in path_info["all_paths"]:
            #     print(f"    - {p}")
            print("")  # 空行分隔